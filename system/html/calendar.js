...
function MonthCalendar(idSpan)
{
    ...
    THIS.objCalendarDay = '';   // 数组
    THIS.objCalendarToday = '';
    THIS.objPrevMonth = '';
    THIS.objNextMonth = '';
    THIS.objCalendarBGOver = ''; // 由于IE对控件显示优先级方面的问题，所以加一背景iframe
    THIS.objCalendarPosLT = ''; // 用于指示左上角位置坐标的控件
    THIS.objCalendarPosRB = ''; // 用于指示右下角位置坐标的控件
    ...
    function Create()
    {
        ...
        try
        {
            ...
            html += '<div style="position:absolute;z-index:7;background-color:' + THIS.colorBG + ';">';
            // 这个span用于取得div的左上角坐标（用于间接取div长宽）
            html += '<span id="' + idCalendarPosLT + '"><span>';
            ...
            html += '</table>';
            html += '<table border="0" width="210"><tr><td width="100%">';
            html += '今天是：<span id="' + idCalendarToday + '"></span>';
            html += '</td><td>';
            // 这个span用于取得div的右下角坐标（用于间接取div长宽）
            html += '<span id="' + idCalendarPosRB + '">&nbsp;<span>';
            html += '</td></tr></table>';
            html += '</div></div>';
            ...
            THIS.objCalendarPosLT = GetObj(idCalendarPosLT);
            THIS.objCalendarPosRB = GetObj(idCalendarPosRB);
            THIS.objCalendarBGOver = GetObj(idCalendarBGOver);
            ...
            for(var i=0 ; i<THIS.objCalendarDay.length ; i++)
            {
                ...
                {
                    THIS.objOwner.value = Format(THIS.thisYear + '-' + THIS.thisMonth + '-' + this.innerHTML);
                    Hide();
                }
                // 鼠标移动
            ...
            {
                THIS.objOwner.value = Format(THIS.objCalendarToday.innerHTML);
                Hide();
            }
    ...
    {
        var posLT = GetPosition(THIS.objCalendarPosLT.id); // 左上角
        var posRB = GetPosition(THIS.objCalendarPosRB.id); // 右下角

        // 修改iframe长宽和日期控件一样
        THIS.objCalendarBGOver.style.width = posRB.x - posLT.x + 11;
        THIS.objCalendarBGOver.style.height = posRB.y - posLT.y + 20;
    }

    function Show()
    {
        // 移到需要显示的位置
        var pos = GetPosition(THIS.objOwner.id); // 需要显示的位置
    ...
    function FillData()
    {
        try
        {
            ...
            // 因为每月的日期天数不同，会导致显示界面大小变化，所在要重新调整背景的iframe
            AdjustBgRect();
        }
        catch(e)
        {
    ...
    function Format(date)
    {
        ...
        re.exec(date);
        return (RegExp.$1 + "-" +
                (RegExp.$2.length==2?RegExp.$2:"0"+RegExp.$2) + "-" +
                (RegExp.$3.length==2?RegExp.$3:"0"+RegExp.$3));
    }
    function Init()
    {
        if(THIS.error != '')
...
History:
 ...
 2007-05-19 : 1.2版（修改做为铺垫的iframe（增加AdjustBgRect()函数）
 2007-05-13 : 1.1版（增加把日期格式化为xxxx-xx-xx形式）
 2007-01-30 : 1.0版
